...
[RequireComponent(typeof(AudioSource))] // Audio moteur ajouté
public class RemoteControlCarController : MonoBehaviour
{
    ...
    [SerializeField]
    private float maxForwardSpeedForPitch = 20f;
    ...
    private void FixedUpdate()
    {
        ...
        if (IsGrounded)
        {
            ...
            {
                float motorForce = motorPowerOverForwardSpeed.Evaluate(Vector3.Dot(ForwardDirection, linearVelocity)) * playerMoveInput.y;
            ...
            if (engineAudioSource != null)
            {
                float speedForAudio = Vector3.Dot(ForwardDirection, linearVelocity);
                float normalizedSpeed = Mathf.Clamp01(speedForAudio / maxForwardSpeedForPitch);
        ...
        else
        {
            ...
            if (Mathf.Abs(playerMoveInput.x) > 0.01f)
            {
                float forwardVelocity = Vector3.Dot(ForwardDirection, linearVelocity);
                float angleOverSpeedRatio = 1.0f;
                if (Mathf.Abs(forwardVelocity) < minForwardSpeedForRotation)
                    angleOverSpeedRatio = Mathf.Abs(forwardVelocity) / minForwardSpeedForRotation;
    ...
    private void Update()
    {
        ...
        if (groundParticles != null && groundParticles.Length > 0)
        {
            ...
            {
                groundParticles[i].SetActive(IsGrounded);
            }
        }
    }
    ...
    private Vector3 ComputeAverageGroundNormal(Vector3 previousAverageGroundNormal)
    {
        ...
        foreach (var points in this.groundContactPoints.Values)
        {
        ...
        if (groundContactCount > 1) averageGroundNormal /= groundContactCount;
    ...
    private void UpdateGroundContact(Collision collision, bool remove = false)
    {
        ...
        {
            if (!groundContactPoints.ContainsKey(collider))
                groundContactPoints.Add(collider, new List<GroundContactPoint>(4));
            ...
            foreach (var contactPoint in contactPoints)
            {
                ...
                if (Vector3.Dot(hitInfo.normal, GravityDirection) < gravityGroundThreshold)
                    points.Add(new GroundContactPoint(contactPoint.point, hitInfo.normal));
            }
        }
    }
    ...
    private void OnCollisionExit(Collision collision) => UpdateGroundContact(collision, true);

    private struct GroundContactPoint
    {
        public readonly Vector3 Point;
        public readonly Vector3 Normal;
        public GroundContactPoint(Vector3 point, Vector3 normal) { Point = point; Normal = normal; }
    }
}